...
| Day 61 | 6 | `getgroups` / `setgroups` / `initgroups` / `getlogin` / `setlogin` / `chroot` | 79 / 80 / 243 / 49 / 50 / 61 | 补充组、登录名与 chroot |
...
53. 对照填写 `notes/daily/day-0N.md`  

完成 Day 7 后再进入第 2 周；不要跳周。下一步：**Day 61** → `getgroups` / `setgroups` / `initgroups` / `getlogin` / `setlogin` / `chroot`。
